...
import { osDetailType, appVersion } from "@/utils/env"; // 确保 appVersion 已导入
...
const Settings = () => {
  ...
  const [ytdlDownloadStatus, setYtdlDownloadStatus] = useConfig(
    "ytdlDownloadStatus",
    "end"
  );
  ...
  const [enableReversedMode, setEnableReversedMode] = useConfig(
    "enableReversedMode",
    false
  );
  ...
  const downloadYtDlp = () => {
    if (osDetailType in YT_DLP.downloadInfo) {
      const ytDlpInfo =
        YT_DLP.downloadInfo[osDetailType as keyof typeof YT_DLP.downloadInfo];
    ...
    <div className={styles.settingsPage} onClick={clickOutside}>
      <div className={styles.container}>
        ...
        <div className={styles.item}>
          <div className="left">
            <div className="title">{t("settings.update.title")}</div>
          </div>
          <div className="right">
            <button onClick={() => setIsCheckingUpdate(true)}>
              {t("settings.update.text")}
            </button>
          </div>
        </div>
        ...
        <section className="unm-configuration">
          ...
          <SettingsInput
            ...
            description={
              <div>
                ...
                <br />
                {t("settings.unm.audioSource.desc3")} <code>,</code>{" "}
          ...
          <div className={styles.item}>
            <div className="left">
              <div className="title">{t("settings.unm.ytdl")}</div>
              {ytdlDownloadStatus != "end" && ytdlDownloadStatus != "error" ? (
                <LinearProgressWithLabel
                  value={ytdlDownloadProgress}
                  downloadspeed={ytdlDownloadSpeed}
                />
              ) : (
                <div className="description">
                  当前版本：{ytdlVersion}；最新版本：{latestVersion},
                  版本不是最新的会导致搜索错误
                </div>
              )}
            </div>
          ...
          <div className="right">
            {isLastfmConnected ? (
              ...
              <button onClick={lastfmConnect}>
                {t("settings.thirdParty.lastfm.authorizeTheConnection")}
              </button>
            )}
          </div>
        </div>
        ...
        <SettingsSelect
          ...
          callback={(proxyProtocol: string) => setProxyProtocol(proxyProtocol)}
        />
        <div
          id="proxy-form"
        ...
        <SettingsSwitch
          ...
          callback={(value: boolean) => setEnableGlobalShortcut(value)}
        />
        <div
          id="shortcut-table"
        ...
        <div className="footer">
          <p className="author">
            MADE BY
            <a href="http://github.com/fangcongyang" target="_blank">
              fangcongyang
            </a>
          </p>
