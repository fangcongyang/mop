...
mod tests {
    ...
    mod translate_url {
        ...
        fn test_translate_url() {
            let mut test_data_1 = "https://www.cloudflare.com/owo".parse::<Url>().unwrap();
            let mut test_data_2 = "https://www.google.com/?search=Rickroll"
                .parse::<Url>()
                .unwrap();

            translate_url(&super::TRANSLATE_MAP, &mut test_data_1).unwrap();
            translate_url(&super::TRANSLATE_MAP, &mut test_data_2).unwrap();

            assert_eq!(test_data_1.to_string(), "https://1.1.1.1/owo");
            assert_eq!(
